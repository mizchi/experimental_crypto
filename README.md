...
| Transport | `tls13` | TLS 1.3 (RFC 8446) key schedule, record layer (AEAD framing + per-record nonce), handshake framing + Finished/CertificateVerify auth, and server-flight parsers (ServerHello/Certificate/CertificateVerify); ClientHello builder + client state machine WIP. |
