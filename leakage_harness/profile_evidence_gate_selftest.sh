...
dudect	native	alpha	3	7.5	2.1	0
dudect	native	beta	3	4.7	1.4	0
...
dudect_thresholds="$tmpdir/dudect-thresholds.tsv"
cat >"$dudect_thresholds" <<'EOF'
# target workload max_observed_abs_t max_mean_abs_t max_failures
* * 10.0 3.0 0
EOF
...
LEAKAGE_EVIDENCE_WORKLOADS="alpha beta" \
  ...
  LEAKAGE_EVIDENCE_DUDECT_TARGETS="native" \
  LEAKAGE_EVIDENCE_MIN_TIMING_RUNS=3 \
  LEAKAGE_EVIDENCE_MIN_DUDECT_RUNS=3 \
  LEAKAGE_EVIDENCE_MIN_CALLGRIND_RUNS=3 \
  LEAKAGE_EVIDENCE_TIMING_THRESHOLDS="$thresholds" \
  LEAKAGE_EVIDENCE_DUDECT_THRESHOLDS="$dudect_thresholds" \
...
summary_no_dudect="$tmpdir/summary-no-dudect.tsv"
grep -v $'^dudect\t' "$summary_good" >"$summary_no_dudect"
LEAKAGE_EVIDENCE_WORKLOADS="alpha beta" \
  LEAKAGE_EVIDENCE_TIMING_TARGETS="native js wasm-gc wasm" \
  LEAKAGE_EVIDENCE_DUDECT_TARGETS="native" \
  LEAKAGE_EVIDENCE_MIN_TIMING_RUNS=3 \
  ...
  LEAKAGE_EVIDENCE_DUDECT_THRESHOLDS="$dudect_thresholds" \
  LEAKAGE_EVIDENCE_CALLGRIND_THRESHOLDS="$callgrind_thresholds" \
  bash "$ROOT/leakage_harness/profile_evidence_gate.sh" "$summary_no_dudect" \
  >/dev/null 2>&1 && fail "missing dudect evidence passed"

summary_low_dudect_runs="$tmpdir/summary-low-dudect-runs.tsv"
sed $'s/dudect\tnative\talpha\t3/dudect\tnative\talpha\t2/' \
  "$summary_good" >"$summary_low_dudect_runs"
LEAKAGE_EVIDENCE_WORKLOADS="alpha beta" \
  LEAKAGE_EVIDENCE_TIMING_TARGETS="native js wasm-gc wasm" \
  LEAKAGE_EVIDENCE_DUDECT_TARGETS="native" \
  LEAKAGE_EVIDENCE_MIN_TIMING_RUNS=3 \
  ...
  LEAKAGE_EVIDENCE_DUDECT_THRESHOLDS="$dudect_thresholds" \
  LEAKAGE_EVIDENCE_CALLGRIND_THRESHOLDS="$callgrind_thresholds" \
  bash "$ROOT/leakage_harness/profile_evidence_gate.sh" "$summary_low_dudect_runs" \
  >/dev/null 2>&1 && fail "low dudect run count passed"

summary_high_dudect="$tmpdir/summary-high-dudect.tsv"
sed $'s/dudect\tnative\talpha\t3\t7.5/dudect\tnative\talpha\t3\t11.0/' \
  "$summary_good" >"$summary_high_dudect"
LEAKAGE_EVIDENCE_WORKLOADS="alpha beta" \
  LEAKAGE_EVIDENCE_TIMING_TARGETS="native js wasm-gc wasm" \
  LEAKAGE_EVIDENCE_DUDECT_TARGETS="native" \
  LEAKAGE_EVIDENCE_MIN_TIMING_RUNS=3 \
  ...
  LEAKAGE_EVIDENCE_DUDECT_THRESHOLDS="$dudect_thresholds" \
  LEAKAGE_EVIDENCE_CALLGRIND_THRESHOLDS="$callgrind_thresholds" \
  bash "$ROOT/leakage_harness/profile_evidence_gate.sh" "$summary_high_dudect" \
  >/dev/null 2>&1 && fail "high dudect t-statistic passed"
...
LEAKAGE_EVIDENCE_WORKLOADS="alpha beta" \
  LEAKAGE_EVIDENCE_TIMING_TARGETS="native js wasm-gc wasm" \
  LEAKAGE_EVIDENCE_MIN_TIMING_RUNS=3 \
  LEAKAGE_EVIDENCE_REQUIRE_DUDECT=0 \
  LEAKAGE_EVIDENCE_REQUIRE_CALLGRIND=0 \
  LEAKAGE_EVIDENCE_TIMING_THRESHOLDS="$thresholds" \
  bash "$ROOT/leakage_harness/profile_evidence_gate.sh" "$summary_no_dudect" \
  >/dev/null
